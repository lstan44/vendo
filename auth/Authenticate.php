...
public function login_seller($email, $pwd_){
        
    $res = array();

    $query = "SELECT pwd from sellers where email ='".$email ."'";
    $stmt = $this->conn->prepare($query);
    $stmt->execute();
    ...
    if($count > 0){
        ...
        $sql = "SELECT seller_id, account_number, firstname, lastname from sellers where pwd='".$pwd_."' ";
        $stmt2 = $this->conn->prepare($sql);
        $stmt2->execute();
        echo "row from first second: ". $stmt2->rowCount() . PHP_EOL;  ///////////// 
